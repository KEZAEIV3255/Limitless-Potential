...
const int SHOCKRIFLE_MAX_CARRY		= 30;
...
class weapon_ofshockrifle : ScriptBasePlayerWeaponEntity
{
	...
	bool Deploy()
	{
		m_flRechargeTime = g_Engine.time + 0.4;
	...
	void Holster( int skiplocal )
	{
		...
		if( m_pPlayer.m_rgAmmo( self.m_iPrimaryAmmoType ) < SHOCKRIFLE_MAX_CARRY )
			m_pPlayer.m_rgAmmo( self.m_iPrimaryAmmoType, SHOCKRIFLE_MAX_CARRY - 13);
	...
	void PrimaryAttack()
	{
		...
		// Play fire sound.
		g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, "weapons/shock_fire.wav", 0.5, ATTN_NORM, 0, 100 );
		...
		if( pShock !is null )
			pShock.pev.velocity	= g_Engine.v_forward * 650;

		m_flRechargeTime = g_Engine.time + 0.3;
