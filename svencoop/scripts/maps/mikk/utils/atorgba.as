RGBA atorgba( const string m_iszFrom )
{
    ...
    if( m_uResult.length() > 0 ) m_uResult[0] = atoi(m_iszSplitColors[0]);
    if( m_uResult.length() > 1 ) m_uResult[1] = atoi(m_iszSplitColors[1]);
    if( m_uResult.length() > 2 ) m_uResult[2] = atoi(m_iszSplitColors[2]);
    if( m_uResult.length() > 3 ) m_uResult[3] = atoi(m_iszSplitColors[3]);
