...
final class MEntityFuncs
{
    ...
    void TriggerDelayed( string m_iszTarget, CBaseEntity@ pActivator, CBaseEntity@ pCaller, int UseTypex )
    {
        ...
        m_Debug.Server( '[MEntityFuncs::Trigger] USE_TYPE \'' + string( UseTypex ) + '\' \'USE_' + ( UseTypex == 0 ? 'OFF' : UseTypex == 1 ? 'ON' : UseTypex == 2 ? 'SET' : UseTypex == 3 ? 'KILL' : 'TOGGLE' ) + '\'', DEBUG_LEVEL_ALMOST );
